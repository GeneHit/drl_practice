...
class ReplayBuffer:
    ...
    def add(
        self, state: NDArray[Any], action: int, reward: float, next_state: NDArray[Any], done: bool
    ) -> None:
...
class DQNAgent(AgentBase[NDArray[Any]]):
    ...
    def save_model(self, pathname: str) -> None:
        """Save model to file."""
        torch.save(
            {"model_state_dict": self.q_network.state_dict(), "device": self.device}, pathname
        )
...
class DQNTrainer(TrainerBase[NDArray[Any]]):
    ...
    def _init_training(self, config: DQNConfig) -> None:
        ...
        self.replay_buffer = ReplayBuffer(config.buffer_size, (self.state_dim,), self.device)
