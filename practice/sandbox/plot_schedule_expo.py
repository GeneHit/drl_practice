...
def plot_exponential_schedule() -> None:
    ...
    for decay_rate, color in zip(decay_rates, colors):
        schedule = ExponentialSchedule(v0=1.0, v1=0.1, decay_rate=decay_rate)
    ...
    for start_e, color in zip(start_values, colors):
        schedule = ExponentialSchedule(v0=start_e, v1=0.1, decay_rate=0.005)
        values = [schedule(int(t)) for t in time_steps]
    ...
    for end_e, color in zip(end_values, colors):
        schedule = ExponentialSchedule(v0=1.0, v1=end_e, decay_rate=0.005)
        values = [schedule(int(t)) for t in time_steps]
    ...
    taxi_schedule = ExponentialSchedule(v0=0.05, v1=0.01, decay_rate=-0.0005)
    ...
    typical_schedule = ExponentialSchedule(v0=1.0, v1=0.05, decay_rate=0.001)
    ...
    fast_schedule = ExponentialSchedule(v0=1.0, v1=0.1, decay_rate=0.01)
...
def plot_negative_decay_analysis() -> None:
    ...
    neg_schedule = ExponentialSchedule(v0=0.05, v1=0.01, decay_rate=-0.0005)
    ...
    pos_schedule = ExponentialSchedule(v0=0.05, v1=0.01, decay_rate=0.0005)
