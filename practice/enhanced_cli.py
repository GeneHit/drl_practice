...
from practice.base import AgentBase, BaseConfig, TrainerBase
...
def load_config_from_file(config_path: str, config_class: Type[BaseConfig]) -> BaseConfig:
    ...
    if config_path.endswith(".json"):
        ...
        config_dict["train_result_filename"] = output_params.get(
            "train_result_filename", "train_result.json"
        )
        config_dict["eval_result_filename"] = output_params.get(
        ...
        for name in dir(config_module):
            ...
            if isinstance(obj, config_class) or (
                hasattr(obj, "__class__") and issubclass(obj.__class__, config_class)
            ):
                return obj
...
def load_model_from_config(
    config: BaseConfig, model_loader: ModelLoader, device: torch.device | None = None
) -> AgentBase[Any]:
...
def push_to_hub(config: BaseConfig, username: str, algorithm_name: str) -> None:
    ...
    repo_id = (
        f"{username}/{config.repo_id}"
        if config.repo_id
        else f"{username}/{algorithm_name.lower()}-model"
    )
...
class ModeHandler:
    ...
    def handle_push_to_hub_mode(
        self, config: BaseConfig, username: str, skip_play: bool = False
    ) -> None:
    ...
    def execute_mode(
        self, mode: str, config: BaseConfig, username: str = "", skip_play: bool = False
    ) -> None:
...
def create_enhanced_parser(
    algorithm_name: str, config_example: str = "config.json"
...
""",
    ...
    parser.add_argument(
        "--config", "-c", type=str, required=True, help="Path to configuration file (.json or .py)"
    )
    ...
    parser.add_argument(
        "--skip_play", action="store_true", help="Skip playing game and generating video"
    )
...
def detect_algorithm_from_config(config_path: str) -> str:
    ...
    if config_path.endswith(".json"):
        ...
        if any(
            key in hyper_params
            for key in ["learning_rate", "epsilon_start", "replay_buffer_capacity", "batch_size"]
        ):
            return "dqn"
...
def get_algorithm_components(
    algorithm: str,
) -> tuple[Type[BaseConfig], Type[TrainerBase[Any]], ModelLoader, Any]:
    ...
    if algorithm == "qtable":
        ...
        from practice.exercise1_qtable.qtable_exercise import QTable, QTableTrainer

        return QTableConfig, QTableTrainer, QTable, None

    elif algorithm == "dqn":
        from practice.exercise2_dqn.dqn_config import DQNConfig
        from practice.exercise2_dqn.dqn_exercise import DQNAgent, DQNTrainer, create_dqn_network
...
def unified_main() -> None:
    ...
    try:
        ...
        config_class, trainer_class, model_loader, network_factory = get_algorithm_components(
            algorithm
        )
