...
class QTableTrainer(TrainerBase[np.int64]):
    ...
    def __init__(self, config: QTableConfig, network: Any = None, device: torch.device | None = None) -> None:
    ...
    def action(self, state: np.int64, episode: int = 0, eval_mode: bool = False, **kwargs: Any) -> ActType:
    ...
    def update(self, state: np.int64, action: ActType, reward: float, next_state: np.int64) -> float:
