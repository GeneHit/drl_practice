...
class DQNConfig(BaseConfig):
    ...
    def validate(self) -> None:
        ...
        if self.replay_buffer_capacity <= 0:
            raise ValueError(f"replay_buffer_capacity must be positive, got {self.replay_buffer_capacity}")
    ...
    def is_2d_observation(self) -> bool:
        ...
        return (
            "use_image" in self.env_kwargs and self.env_kwargs["use_image"] or
            "resize_shape" in self.env_kwargs or
            "frame_stack_size" in self.env_kwargs
        ) 