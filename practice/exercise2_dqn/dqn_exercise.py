...
class ReplayBuffer:
    ...
    def add(
        ...
        for i in range(batch_size):
            ...
            self.next_states[idx] = torch.from_numpy(next_states[i]).to(
...
class DQNAgent(AgentBase[NDArray[Any]]):
    ...
    def save_model(self, pathname: str) -> None:
        """Save model to file."""
        torch.save(
            {"model_state_dict": self.q_network.state_dict(), "device": str(self.device)}, pathname
        )
    ...
    def load_from_checkpoint(cls, pathname: str, device: torch.device | None = None) -> "DQNAgent":
        ...
        first_layer_key = [
            k for k in model_state.keys() if "network.0.weight" in k or "conv.0.weight" in k
        ][0]
        ...
        if "conv" in first_layer_key:
            ...
            raise NotImplementedError(
                "Loading 2D networks requires saving input shape configuration"
            )
        else:
            # This is a 1D network
            first_layer = model_state["network.0.weight"]
            state_dim = first_layer.shape[1]
            action_dim = model_state[
                "network."
                + str(
                    len([k for k in model_state.keys() if "network." in k and ".weight" in k]) * 2
                    - 2
                )
                + ".weight"
            ].shape[0]
...
class DQNTrainer(TrainerBase[NDArray[Any]]):
    ...
    def train(self, log_dir: str) -> DQNAgent:
        ...
        self.replay_buffer = ReplayBuffer(
            self.config.replay_buffer_capacity, obs_shape, self.device
        )
