...
def _minimal_dueling_dqn_config(temp_output_dir: Path) -> DQNConfig:
    """Return a minimal DQNConfig for fast testing of dueling DQN."""
...
def test_dueling_dqn_trainer_basic_flow(temp_output_dir: Path) -> None:
    """Test DuelingDQNTrainer training flow with minimal config and no file saving."""
    config = _minimal_dueling_dqn_config(temp_output_dir)
...
def test_dueling_dqn_config_validation() -> None:
    """Test that dueling DQN config has correct algorithm type."""
    config = get_app_config()
    # Note: dueling DQN uses basic algorithm but with DuelingDQN1D network
    assert config.dqn_algorithm == "basic", (
        "Config should specify basic DQN algorithm for dueling DQN"
    )


def test_dueling_dqn_context_generation() -> None:
    """Test that context generation works correctly for dueling DQN."""
    config = get_app_config()
    context = generate_context(config)
...
def test_dueling_dqn_minimal_training(temp_output_dir: Path) -> None:
    """Test minimal dueling DQN training to ensure no errors occur."""
    config = _minimal_dueling_dqn_config(temp_output_dir)
    context = generate_context(config)
...
def test_dueling_dqn_training_with_accelerated_device(temp_output_dir: Path) -> None:
    """Test dueling DQN training flow with CUDA or MPS if available."""
...
def test_dueling_dqn_network_architecture() -> None:
    """Test that the dueling DQN network has the correct architecture."""
    config = get_app_config()
    context = generate_context(config)
    ...
    try:
        ...
        # Verify the network has the expected dueling architecture components
        assert hasattr(network, "feature"), "Network should have feature stream"
        assert hasattr(network, "value_head"), "Network should have value head"
        assert hasattr(network, "advantage_head"), "Network should have advantage head"

        # Verify the network can process input and produce output
        batch_size = 4
        state_dim = 8  # LunarLander state dimension
        action_dim = 4  # LunarLander action dimension

        # Create dummy input
        dummy_input = torch.randn(batch_size, state_dim, device=config.device)
        ...
        # Verify output is a valid Q-value tensor
