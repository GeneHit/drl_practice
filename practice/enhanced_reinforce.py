...
from practice.utils.schedules import LinearSchedule, ConstantSchedule
...
class EnhancedReinforceAgent(AgentBase[NDArray[Any]]):
    ...
    @classmethod
    def load_from_checkpoint(cls, pathname: str, device: torch.device | None = None) -> "EnhancedReinforceAgent":
...
class EpisodeBuffer:
    ...
    def add(self, state: NDArray[Any], action: int, reward: float, log_prob: torch.Tensor, entropy: torch.Tensor, intrinsic_reward: float = 0.0) -> None:
    ...
    def get_returns(self, gamma: float, use_intrinsic: bool = False) -> list[float]:
        ...
        rewards = self.rewards if not use_intrinsic else [r + ir for r, ir in zip(self.rewards, self.intrinsic_rewards)]
...
class EnhancedReinforceTrainer(TrainerBase[NDArray[Any]]):
    ...
    def compute_intrinsic_reward(self, state: NDArray[Any], action: int, next_state: NDArray[Any], config: EnhancedReinforceConfig) -> float:
        ...
        for episode in tqdm(range(config.episodes), desc="Training Enhanced REINFORCE"):
            ...
            for step in range(config.max_steps):
                ...
                if self.curiosity_module is not None:
                    ...
                    curiosity_loss = self.curiosity_module.update(state_tensor, action_tensor, next_state_tensor)
            ...
            # Update baseline
            total_return = episode_reward + episode_intrinsic_reward if config.use_curiosity else episode_reward
