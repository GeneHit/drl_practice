...
def create_env_from_config(
    ...
    """Create environment from environment parameters.
...
def train_mode(cfg_data: dict[str, Any], skip_play: bool = False) -> None:
    """Train the Q-learning model, optionally play game and generate video."""
...
def main() -> None:
    """Main function to handle different modes based on command line arguments."""
    parser = argparse.ArgumentParser(
        description="Q-learning Main Script with Multiple Modes",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
            Mode descriptions:
            train (default): Train the Q-learning model, then play game and generate video
            ...
            python q_main.py --config config.json

            # Train mode without play/video generation
            python q_main.py --config config.json --skip_play

            # Push to hub with play/video
            python q_main.py --config config.json --mode push_to_hub --username myuser

            # Push to hub without play/video
            python q_main.py --config config.json --mode push_to_hub --username myuser --skip_play

            # Only play and generate video
            python q_main.py --config config.json --mode play_only
        """,
    )
