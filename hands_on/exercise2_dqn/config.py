...
class DQNTrainConfig:
    ...
    def _validate_parameters(self) -> None:
        ...
        if not isinstance(self.timesteps, int) or self.timesteps <= 0:
            raise ValueError(f"timesteps must be a positive integer, got {self.timesteps}")

        if not isinstance(self.replay_buffer_capacity, int) or self.replay_buffer_capacity <= 0:
            raise ValueError(
        ...
        if not isinstance(self.target_update_interval, int) or self.target_update_interval <= 0:
            raise ValueError(
                f"target_update_interval must be a positive integer, got {self.target_update_interval}"
            )

        if not isinstance(self.update_start_step, int) or self.update_start_step <= 0:
            raise ValueError(
                f"update_start_step must be a positive integer, got {self.update_start_step}"
            )

        # Validate probability ranges (0-1)
        if not isinstance(self.start_epsilon, (int, float)) or not 0 <= self.start_epsilon <= 1:
            raise ValueError(f"start_epsilon must be between 0 and 1, got {self.start_epsilon}")

        if not isinstance(self.end_epsilon, (int, float)) or not 0 <= self.end_epsilon <= 1:
            raise ValueError(f"end_epsilon must be between 0 and 1, got {self.end_epsilon}")
        ...
        if self.start_epsilon < self.end_epsilon:
            raise ValueError("start_epsilon should be >= end_epsilon for exploration decay")
