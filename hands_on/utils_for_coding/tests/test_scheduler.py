    ...
    def test_linear_schedule_properties(
        ...
        if schedule_type == "decreasing":
            ...
            for t in range(duration, duration * 2, max(1, duration // 20)):
                assert schedule(t) <= end_e + 1e-10, (
                    f"Value at t={t} should not exceed end_e={end_e}"
                )
        ...
        assert abs(schedule(duration + 50) - end_e) < 1e-10, (
            f"Beyond duration, should return end_e={end_e}"
        )
